...
begin
 ...
 //itzik This diables the move to right or left
 //  if goRowSelect in Options then FCurrent.X := FixedCols;
    if Show then ClampInView(FCurrent);  // itzik - so that the grid moves with the cell
    SelectionMoved(OldSel);
    ...
    begin
       with OldCurrent do InvalidateCell(X-1, Y-1);
       with FCurrent do InvalidateCell(ACol, ARow);
     end;  
  end;
end;
...
begin
  ...
  if ssCtrl in Shift then
    ...
    case Key of
      ...
      VK_LEFT:
        if goRowSelect in Options then
          Dec(NewCurrent.Y, RTLFactor) else //itzik Org src =  Dec(NewCurrent.Y, RTLFactor) else
          Dec(NewCurrent.X, RTLFactor);
      VK_RIGHT:
        if goRowSelect in Options then
          Inc(NewCurrent.Y, RTLFactor) else //itzik Org src =  Inc(NewCurrent.Y, RTLFactor) else
