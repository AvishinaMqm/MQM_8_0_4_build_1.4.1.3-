unit FMPerformanceReport;

interface

uses
  Windows, Messages, SysUtils, Classes, Graphics, Controls, Forms, Dialogs,
  ComCtrls, ExtCtrls, StdCtrls, Buttons, Menus, UMSchedList;

type

  TFPerformanceReport = class(TForm)
    TVReport: TTreeView;
    LVReport: TListView;
    Splitter1: TSplitter;
    PnlBtm: TPanel;
  ...
  public
    constructor CreatePerformanceRpt(AOwner: Tcomponent; Objs: TMSchedList);
...
var
  FPerformanceReport: TFPerformanceReport;
...
constructor TFPerformanceReport.CreatePerformanceRpt(AOwner: Tcomponent; Objs: TMSchedList);
begin
...
//  TVReport.Color := s_colors[PlanBackGrd];
  ...
  FPerformanceReport := self;
end;

//----------------------------------------------------------------------------//

destructor TFPerformanceReport.Destroy;
begin
  inherited Destroy;

//  FMQMPlan.RefreshActiveTab;
  FPerformanceReport := nil;
end;

//----------------------------------------------------------------------------//

procedure TFPerformanceReport.FormClose(Sender: TObject;
...
procedure TFPerformanceReport.SetColForPlan;
var
  TempCol: TListColumn;
begin
...
procedure TFPerformanceReport.SetColForBin;
var
  TempCol: TListColumn;
begin
  LVReport.Columns.Clear;
...
procedure TFPerformanceReport.InitTreeView;
{
var
  i, TotLines : integer;
...
{
  TotLines := m_Objs.Count + m_ObjsInBin.Count;
  ...
  begin
    ...
    Lev2Tn := TVReport.Items.AddChild(Lev1Tn, _('Below lowest start date'));
...
//      TList(Lev2Tn.Data).Sort(@ProcSortProdEff)
    ...
    Lev2Tn := TVReport.Items.AddChild(Lev1Tn, _('Within lowest start date tolerance'));
    for i := 0 to m_ObjsOnPlan.Count-1 do
    begin
      ReportRec := PTAutoSchedResult(m_ObjsOnPlan.items[i]);
      p_sc.GetDatesInfo(ReportRec.m_ObjID, DtInfo);
...
//      TList(Lev2Tn.Data).Sort(@ProcSortProdEff)
    ...
    Lev2Tn := TVReport.Items.AddChild(Lev1Tn, _('Within highest end date tolerance'));
    for i := 0 to m_ObjsOnPlan.Count-1 do
    begin
      ReportRec := PTAutoSchedResult(m_ObjsOnPlan.items[i]);
      p_sc.GetDatesInfo(ReportRec.m_ObjID, DtInfo);
...
//      TList(Lev2Tn.Data).Sort(@ProcSortProdEff)
    ...
    Lev2Tn := TVReport.Items.AddChild(Lev1Tn, _('Over highest end date tolerance'));
    for i := 0 to m_ObjsOnPlan.Count-1 do
    begin
      ReportRec := PTAutoSchedResult(m_ObjsOnPlan.items[i]);
      p_sc.GetDatesInfo(ReportRec.m_ObjID, DtInfo);
...
procedure TFPerformanceReport.TVReportChange(Sender: TObject; Node: TTreeNode);
var
  ReportRec: PTAutoSchedResult;
  i: integer;
  ListItem: TListItem;
...
procedure TFPerformanceReport.LVReportCustomDrawItem(Sender: TCustomListView;
  Item: TListItem; State: TCustomDrawState; var DefaultDraw: Boolean);
//var
//  PlanType : TPlanType;
//  ObjId: TSchedID;
...
procedure TFPerformanceReport.LVReportSelectItem(Sender: TObject;
...
procedure TFPerformanceReport.FormCreate(Sender: TObject);
begin
...
procedure TFPerformanceReport.BtnCancClick(Sender: TObject);
begin
...
procedure TFPerformanceReport.JobDetail1Click(Sender: TObject);
var
  ObjId: TSchedID;
...
procedure TFPerformanceReport.BtnOkClick(Sender: TObject);
begin
  Close;
  ModalResult := mrOk;
end;
